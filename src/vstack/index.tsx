...
// Breakpoints mirror @kreftforeningen/web-css tokens but are hard-coded so media queries stay valid.
const breakpoints = {
  sm: "40rem",
  md: "48rem",
  lg: "64rem",
...
const propertyConfig = {
  ...
  justify: { css: "justify-content", fallback: "flex-start" },
  align: { css: "align-items", fallback: "stretch" },
...
const responsiveBaseStyles = Object.entries(propertyConfig)
  .map(
    ([key, value]) =>
      `    ${value.css}: var(--kf-vstack-${key}-base, ${value.fallback});`
  )
  .join("\n");
...
${Object.entries(propertyConfig)
  .map(
    ([key, value]) =>
      `    ${value.css}: var(--kf-vstack-${key}-${bp}, var(--kf-vstack-${key}-base, ${value.fallback}));`
  )
  .join("\n")}
  }
}`
...
const VStackGlobalStyles = createGlobalStyle`
  .kf-vstack {
...
const assignResponsiveVars = <T,>(
  ...
  if (!isResponsiveObject(value)) {
    styles[`--kf-vstack-${key}-base`] = formatter(value);
    return styles as React.CSSProperties;
  }

  if (value.base !== undefined) {
    styles[`--kf-vstack-${key}-base`] = formatter(value.base);
  }

  (Object.keys(breakpoints) as Breakpoint[]).forEach((bp) => {
    if (value[bp] !== undefined) {
      styles[`--kf-vstack-${key}-${bp}`] = formatter(value[bp] as T);
    }
  });
...
type VStackProps<T extends React.ElementType = "div"> = {
  as?: T;
  asChild?: boolean;
  ...
  (
    ...
    return (
      <Comp
        ref={ref}
        data-slot="vstack"
        className={cn("kf-vstack", className)}
